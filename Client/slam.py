...
def point2plane_registration(source, target, distance_threshold, trans_init): 
    cauchyLoss = o3d.pipelines.registration.CauchyLoss(k=0.1)
    tukeyLoss = o3d.pipelines.registration.TukeyLoss(k=0.25)
    #ROBUST KERNEL WITH TUKEY LOSS AND APROX INITIAL TRANSFORMATION MATRIZ
    #************************************************************************************
    icp_coarse = o3d.pipelines.registration.registration_icp(
        source, target, distance_threshold, trans_init,
        o3d.pipelines.registration.TransformationEstimationPointToPlane(tukeyLoss))
    #************************************************************************************
    #REFINED FINAL TRANSFORMATION MATRIZ
    icp_fine = o3d.pipelines.registration.registration_icp(source, target, 0.5*distance_threshold,
        icp_coarse.transformation,
        o3d.pipelines.registration.TransformationEstimationPointToPlane(cauchyLoss))
    #************************************************************************************
    #FINAL MATRIZ WITH THE COMPLETE TRANSFORMATIONS AND INFORMATION
    information_icp = o3d.pipelines.registration.get_information_matrix_from_point_clouds(
        source, target, 0.5*distance_threshold,
...
class SLAM():
    def __init__(self, voxel_size, color_mode = False):
    ...
    def update(self, cloud):
        ...
        if not len(self.keyframes):
            ...
            self.frustum = o3d.geometry.LineSet.create_camera_visualization(intrinsic,odom_inv, 1)
            return
    ...
    def update_keyframe(self, cloud, fpfh):
        success, transformation, information = register_point_cloud_fpfh(
            ...
            self.voxel_size, True, color_mode= self.color_mode
        )
        ...
        for i in range(0, len(self.keyframes) - 5, 5):
            transformation = np.dot(np.linalg.inv(self.keyframes[i].odom), self.keyframes[-1].odom)
            ...
            if trans < 1 and rot < (np.pi / 18):
                success, transformation, information = register_point_cloud_fpfh(
                    ...
                    self.voxel_size, False, color_mode= self.color_mode
                )
                if success:
                    edge = o3d.pipelines.registration.PoseGraphEdge(
    ...
    def save_trajectory(self):
        for i in range(len(self.keyframes)):
            try:
                self.traj.append(self.graph.nodes[i].pose)
...
if __name__ == '__main__':
    ...
    for i in range(0, num_frames, skip_frames):
        ...
        source = o3d.io.read_point_cloud(f"data\\pointClouds\\my_pcd{i:04d}.ply") 
        source.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.25, max_nn=25))
        source  = o3d.geometry.PointCloud.farthest_point_down_sample(source,int(0.1*np.asanyarray(source.points).shape[0]))
